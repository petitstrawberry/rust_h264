...
fn main() {
    ...
    if cbp_code != 3 {  // cbp_intra[0]=47 != 0
    ...
    let code16: u32 = bits.iter().take(16).fold(0u32, |acc, &b| (acc << 1) | b as u32);
    println!("16-bit code: 0b{:016b} = {}", code16, code16);
    
    // Look for matches at different lengths
    for len in 1..=16 {
        let code: u32 = bits.iter().take(len).fold(0u32, |acc, &b| (acc << 1) | b as u32);
